...
Architecture:
  - Doc detector: compare FP32 YOLO vs TFLite int8 via Ultralytics YOLO.val()
    (kept as scaffolding for a future CUDA-trained doc detector; on Apple
    Silicon doc segmentation runs via VNDetectDocumentSegmentationRequest /
    ML Kit Document Scanner — see ARCHITECTURE_DECISIONS.md)
...
Usage:
    ...
    python model-training/export/validate_quantization.py --model field_detector
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        choices=["doc_detector", "field_detector", "all"],
        default="all",
