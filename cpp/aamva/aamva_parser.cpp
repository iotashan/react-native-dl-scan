...
std::optional<LicenseData> parse_aamva(const std::string& raw_data) {
    ...
    // PDF417 is a digital decode of the AAMVA-encoded string the issuing
    // authority wrote at issuance — no OCR, no image recognition, no
    // bbox-to-text matching. Every populated field is by definition
    // CrossValidated (1.00). Without this, the JS side sees an empty
    // dataConfidence map for barcode scans and the ResultView UI shows
    // no confidence pills — making PDF417 scans look LESS trusted than
    // the OCR path, which is the opposite of reality. Task #43 carry-over,
    // surfaced when user flipped to the barcode side 2026-05-10.
    static constexpr float kPdf417Score =
        to_score(ValidationTier::CrossValidated);
    auto stamp = [&](const char* key, const std::optional<std::string>& v) {
        if (v.has_value() && !v->empty()) {
            result.fieldConfidence[key] = kPdf417Score;
        }
    };
    stamp("firstName",      result.firstName);
    stamp("lastName",       result.lastName);
    stamp("middleName",     result.middleName);
    stamp("licenseNumber",  result.licenseNumber);
    stamp("dateOfBirth",    result.dateOfBirth);
    stamp("expirationDate", result.expirationDate);
    stamp("issueDate",      result.issueDate);
    stamp("sex",            result.sex);
    stamp("eyeColor",       result.eyeColor);
    stamp("hairColor",      result.hairColor);
    stamp("height",         result.height);
    stamp("weight",         result.weight);
    stamp("street",         result.street);
    stamp("city",           result.city);
    stamp("state",          result.state);
    stamp("postalCode",     result.postalCode);
    stamp("country",        result.country);
    stamp("vehicleClass",   result.vehicleClass);
    stamp("restrictions",   result.restrictions);
    stamp("endorsements",   result.endorsements);
