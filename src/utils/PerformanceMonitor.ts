...
export class PerformanceMonitor {
  ...
  checkpoint(name: string, metadata?: Record<string, any>): void {
    if (!this.activeSession) {
      logger.warn('Performance checkpoint called without active session', {
        name,
      });
      return;
    }
  ...
  private validatePerformanceTargets(metrics: PerformanceMetrics): void {
    ...
    if (
      ...
      metrics.ocrProcessingTime > this.targets.ocrProcessingMs
    ) {
      alerts.push({
        type: 'critical',
    ...
    if (
      metrics.peakCpuUtilization &&
      metrics.peakCpuUtilization > this.targets.cpuUtilizationPercent
    ) {
      alerts.push({
        type: 'warning',
        category: 'cpu',
   ...
   */
  private storeSessionData(_metrics: PerformanceMetrics): void {
  ...
  private calculatePerformanceSummary(
    ...
    const sortedByTotal = results.sort(
      (a, b) => a.totalProcessingTime - b.totalProcessingTime
    );
    const count = results.length;

    const mean = this.calculateMeanMetrics(results);
    const median = sortedByTotal[Math.floor(count / 2)] || mean;
    ...
    return {
      mean,
      median,
      p95: sortedByTotal[p95Index] || sortedByTotal[count - 1] || mean,
      p99: sortedByTotal[p99Index] || sortedByTotal[count - 1] || mean,
    };
  }
  ...
  private calculateMeanMetrics(
    ...
    const sum = results.reduce(
      (acc, result) => ({
        totalProcessingTime:
          acc.totalProcessingTime + result.totalProcessingTime,
        ocrProcessingTime:
          acc.ocrProcessingTime + (result.ocrProcessingTime || 0),
        modeTransitionTime:
          acc.modeTransitionTime + (result.modeTransitionTime || 0),
        memoryDeltaMB: acc.memoryDeltaMB + result.memoryDeltaMB,
        peakCpuUtilization:
          acc.peakCpuUtilization + (result.peakCpuUtilization || 0),
   ...
   */
  private sanitizeMetricsForLogging(
...
class PerformanceSession {
  ...
  constructor(id: string, type: 'barcode' | 'ocr' | 'fallback') {
    ...
    this._startMemory = this.getCurrentMemoryUsage();
  }
  ...
  generateMetrics(memoryBaseline: number): PerformanceMetrics {
    ...
    return {
      ...
      peakMemoryUsageMB: Math.max(
        ...this.memoryEvents.map((e) => e.usage),
        finalMemory
      ),
      finalMemoryUsageMB: finalMemory,
      memoryDeltaMB: memoryDelta,
      memoryAllocations: this.memoryEvents.filter((e) => e.size && e.size > 0)
        .length,
      memoryDeallocations: this.memoryEvents.filter((e) => e.size && e.size < 0)
        .length,

      peakCpuUtilization:
        this.cpuSamples.length > 0 ? Math.max(...this.cpuSamples) : undefined,
      averageCpuUtilization:
        this.cpuSamples.length > 0
          ? this.cpuSamples.reduce((a, b) => a + b, 0) / this.cpuSamples.length
          : undefined,
      peakGpuUtilization:
        this.gpuSamples.length > 0 ? Math.max(...this.gpuSamples) : undefined,
      ...
      frameProcessingRate:
        this.frameMetrics.rates.length > 0
          ? this.frameMetrics.rates.reduce((a, b) => a + b, 0) /
            this.frameMetrics.rates.length
          : undefined,
  ...
  private extractTimingBreakdowns(): Partial<PerformanceMetrics> {
    ...
    const checkpointTimes = Array.from(this.checkpoints.entries()).map(
      ([name, data]) => ({
        name,
    ...
    for (let i = 0; i < checkpointTimes.length; i++) {
      ...
      if (current && current.name.endsWith('_start') && next) {
