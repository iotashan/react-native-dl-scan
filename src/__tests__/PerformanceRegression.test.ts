...
const REGRESSION_THRESHOLDS = {
  timing: 0.2, // 20% slower
  memory: 0.3, // 30% more memory
  cpu: 0.25, // 25% more CPU
  failure_rate: 0.1, // 10% more failures
};
...
describe('Performance Regression Tests', () => {
  ...
  describe('Baseline Performance Validation', () => {
    it('should establish baseline for barcode scanning performance', async () => {
      ...
      const p95 =
        timings[Math.floor(timings.length * 0.95)] ||
      ...
      expect(p50).toBeLessThan(
        baseline.p50 * (1 + REGRESSION_THRESHOLDS.timing)
      );
      expect(p95).toBeLessThan(
        baseline.p95 * (1 + REGRESSION_THRESHOLDS.timing)
      );
      expect(p99).toBeLessThan(
        baseline.p99 * (1 + REGRESSION_THRESHOLDS.timing)
      );
  ...
  describe('Regression Detection', () => {
    it('should detect timing performance regression', async () => {
      ...
      if (hasRegression) {
        console.warn(
          `Timing regression detected: ${p95}ms > ${regressionThreshold}ms threshold`
        );
      }
    ...
    it('should detect memory usage regression', async () => {
      ...
      const largeMockOCRData: OCRTextObservation[] = Array.from(
        { length: 15 },
        (_, i) => ({
      ...
      // Track maximum memory delta for analysis
      Math.max(...validMemoryDeltas);
      const avgMemoryDelta =
        validMemoryDeltas.reduce((a, b) => a + b, 0) / validMemoryDeltas.length;

      // Check for memory regression
      const memoryRegressionThreshold =
        baseline.memory_delta_mb * (1 + REGRESSION_THRESHOLDS.memory);
      ...
      if (avgMemoryDelta > memoryRegressionThreshold) {
        console.warn(
          `Memory regression detected: ${avgMemoryDelta}MB > ${memoryRegressionThreshold}MB threshold`
        );
      }
    });
    ...
    it('should track performance trends over time', async () => {
      ...
      for (let run = 0; run < 3; run++) {
        ...
        if (sessionResults.length > 0) {
          const timings = sessionResults
            .map((r) => r.totalProcessingTime)
            .filter((t) => t > 0);
          if (timings.length > 0) {
            // Calculate average timing for trend analysis
  ...
  describe('Performance Stability', () => {
    it('should maintain consistent performance under load', async () => {
      ...
      const variance =
        validResults.reduce((acc, val) => acc + Math.pow(val - mean, 2), 0) /
        validResults.length;
      ...
      if (coefficientOfVariation > 0.3) {
        console.warn(
          `Performance variability detected: CV = ${(coefficientOfVariation * 100).toFixed(1)}%`
        );
      }
    });
  ...
  describe('Resource Usage Regression', () => {
    it('should not regress in CPU utilization', async () => {
      ...
      if (cpuUsageResults.length > 0) {
        const avgCpuUsage =
          cpuUsageResults.reduce((a, b) => a + b, 0) / cpuUsageResults.length;
        ...
        if (avgCpuUsage > regressionThreshold) {
          console.warn(
            `CPU usage regression detected: ${avgCpuUsage}% > ${regressionThreshold}% threshold`
          );
        }
      }
    });
  ...
  describe('Automated Regression Reporting', () => {
    it('should generate comprehensive regression report', async () => {
      ...
      const performanceImprovements = Object.entries(testResults).filter(
        ([_, test]: [string, any]) =>
          test.avg ? test.avg < test.baseline : test.p95 < test.baseline
      );
      ...
      const regressionReport = {
        ...
        tests_passed: Object.keys(testResults).filter(
          (key) => testResults[key as keyof typeof testResults].passed
        ).length,
