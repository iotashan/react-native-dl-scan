...
export function calculateQualityMetrics(_frame: Frame): RealTimeQualityMetrics {
  ...
  const blurStatus = blurValue < 0.15 ? 'good' : blurValue < 0.3 ? 'warning' : 'poor';
  ...
  const lightingStatus = brightness > 0.6 && uniformity > 0.7 ? 'good' : 
                        brightness > 0.4 && uniformity > 0.5 ? 'warning' : 'poor';
  ...
  const distance = distanceRandom < 0.2 ? 'too_close' : 
                  distanceRandom > 0.8 ? 'too_far' : 'optimal';
  
  const positioningStatus = documentDetected && alignment > 0.7 && distance === 'optimal' ? 'good' :
                           documentDetected && alignment > 0.4 ? 'warning' : 'poor';
  
  // Calculate overall score
  const overallScore = documentDetected ? 
    ((1 - blurValue) + brightness + alignment) / 3 : 0.2;
  
  const readyToScan = overallScore > 0.7 && 
                     blurStatus === 'good' && 
                     lightingStatus !== 'poor' && 
...
export function convertLegacyMetrics(legacy: {
  ...
  return {
    blur: {
      value: legacy.blur,
      status: legacy.blur < 0.3 ? 'good' : legacy.blur < 0.6 ? 'warning' : 'poor',
    },
    lighting: {
      brightness: legacy.lighting,
      uniformity: legacy.lighting * 0.9, // Estimate uniformity
      status: legacy.lighting > 0.6 ? 'good' : legacy.lighting > 0.3 ? 'warning' : 'poor',
    },
    positioning: {
      documentDetected: legacy.positioning > 0.3,
      alignment: legacy.positioning,
      distance: legacy.positioning > 0.8 ? 'optimal' : 
               legacy.positioning > 0.5 ? 'too_far' : 'too_close',
      status: legacy.positioning > 0.7 ? 'good' : legacy.positioning > 0.4 ? 'warning' : 'poor',
    },
    overall: {
 ...
 */
export function shouldAnalyzeFrame(frameCount: number, targetFPS: number = 10): boolean {
  'worklet';
 ...
 */
const qualityCache = new Map<string, { metrics: RealTimeQualityMetrics; timestamp: number }>();

export function getCachedQualityMetrics(frameId: string, maxAge: number = 100): RealTimeQualityMetrics | null {
  'worklet';
...
export function setCachedQualityMetrics(frameId: string, metrics: RealTimeQualityMetrics): void {
  'worklet';
